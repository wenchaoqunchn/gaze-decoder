...
- `participants/`
  - `P1/` … `P8/`: participant folders (anonymized IDs)
    ...
    - `split_data/`: gaze samples split by view instance
      - `split_data_01.csv`, `split_data_02.csv`, …
  - `metrics.csv`: AOI-level metrics
    - Notes:
      - **One row = one AOI within one view instance**
      - `view` may repeat across instances; use `view_instance` / `view_instance_id` to distinguish repeated visits
